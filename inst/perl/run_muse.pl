...
# if no job name provided, set to muse_SAMPLE_ID.vcf
if( not $job_name ) { 
	my $job_name = "muse_${sample_id}";
}
...
my $muse = $config->{muse_path};
my $dbsnp = $config->{dbsnp_vcf};
...
#SCRIPT
my $sample_muse_script="${code_directory}/${job_name}.sh";

open(OUT,">$sample_muse_script") or die "Cannot open output script $!\n";
print OUT <<EOF;
${cluster_settings}

mkdir -p ${output_directory} ${log_directory} ${code_directory}

${muse} call -f ${reference_genome} \\
-O ${output_directory}/${sample_id} \\
${tumour_bam} \\
${normal_bam}

${muse} sump -I ${output_directory}/${sample_id}.MuSE.txt -E \\
...
EOF
	...
	close(OUT);
if ($cluster_scheduler eq "LSF") {
  system("bsub < $sample_muse_script");
} elsif ($cluster_scheduler eq "PBS") {
  system("qsub $sample_muse_script");
}
